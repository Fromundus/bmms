...
export default function AccountsTable() {
  ...
  const updateRole = async (ids: number[], role: string) => {
    ...
    try {
      await api.put("/users/role", { ids, role });
  ...
  const updateStatus = async (ids: number[], status: string) => {
    ...
    try {
      await api.put("/users/status", { ids, status });
  ...
  const resetPasswordDefault = async (id: number) => {
    ...
    try {
      ...
      await api.put('/users/reset-password-default/', data);
  ...
  const verifyEmail = async (id: number) => {
    setLoading(true);
    try {
      const res = await api.post("/email-verify", { id });
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{counts.driver}</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                  <>
                    ...
                    <IconButton className={`bg-blue-500 hover:bg-blue-600 text-white`} onClick={() => {
                      ...
                      }} disabled={selected.length === 0 || loading || !bulkRole}>
                      <Save />
                    </IconButton>
                ...
                users.map((u) => (
                  <TableRow key={u.id}>
                    ...
                    <TableCell>{u.status === "active" ? 
                      ...
                      :
                      <Badge className="text-nowrap" variant="destructive">Inactive</Badge>
                    }</TableCell>
                    ...
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {u.status === "inactive" && <DropdownMenuItem onClick={() => updateStatus([u.id], "active")}>
                            Activate
                          </DropdownMenuItem>}
                          ...
                          <DropdownMenuItem onClick={() => resetPasswordDefault(u.id)}>
                            Reset Password
                          </DropdownMenuItem>
